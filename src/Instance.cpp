...
void Instance::Get_data_file(std::string name){
    ...
    int file_line_position, id = 0;
    float lenght_aux, point_x, point_y, point_score, point_service_time, point_opening_time, point_closing_time;
    Point point_aux;
        ...
        if(!file_line.empty()){
            ...
            switch(file_line_position){//get a line from file, convert to an int and put on correct variable
                case 1://get vertices, extra hotels and trips
                    ...
                    trips.resize(num_trips);
                    break;
                case 2://tour maximum length
                    iss >> tour_length;
                    break;
                case 3://each trip length
                    while (iss >> lenght_aux) trip_length.push_back(lenght_aux);
                    break;
